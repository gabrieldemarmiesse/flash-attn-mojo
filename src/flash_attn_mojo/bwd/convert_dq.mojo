"""Backward convert-dQ kernel: sum fp32 dqaccum slots → cast → write dq.

Reads the deterministic-slot fp32 `dqaccum[num_n_blocks, B, H, L, D]`
workspace that the main bwd kernel writes per (n_block) without
atomics (each kv-block-grid block owns its own slot), sums across the
`num_n_blocks` dim, casts each element down to the output dtype (bf16
or fp16), and writes it to the user's `dq[B, L, H, D]` tensor — note
the layout transpose (H and L swapped). Mirrors Tri Dao's
...
Each block owns the (BM, D) slice
`dqaccum[:, b, h, q_tile_row_base:..., :]` and writes the matching
`dq[b, q_tile_row_base:..., h, :]` slice. The 128 threads strip-mine
the BM*D fp32 reads / dtype writes with stride 128 (each thread
handles (BM*D)/128 elements: 16 at D=32, 32 at D=64, 64 at D=128).
Rows past `seq_len` are skipped via the bounds check. The inner loop
walks the n_block dim and accumulates in fp32.

No shared memory, no atomics — pure per-element accumulate-and-cast.
"""
    ...
    for i in range(elts_per_thread):
        ...
        if q_row < seq_len:
            ...
            for n_block in range(num_n_blocks):
                acc = acc + (
                    dqaccum_ptr + n_block * dqaccum_n_stride + cell_off
                )[0]
            var dst_idx: Int = dq_bh_base + q_row * dq_l_stride + col
            (dq_ptr + dst_idx)[0] = acc.cast[dtype]()
