...
This module vendors exactly the one shape the kernels need —
m64n128k16, f32 accum — with the instruction suffix switched to f16.
The asm body, register packing, and constraint construction mirror
the stdlib's n == 128 arm 1:1 so the f16 codegen matches the proven
bf16 path. Used ONLY under ``comptime dtype == DType.float16`` forks
at the three RS call sites (fwd PV, bwd dV, bwd dK); bf16 keeps the
stdlib path byte-identical.
"""
