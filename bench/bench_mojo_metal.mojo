# bench_mojo_metal: v0 flash-attention FORWARD on Apple-silicon GPUs
...
#
# v0 is the correctness + harness milestone, NOT the fast kernel: the
# pinned mojo 1.0.0b1 exposes no usable matrix instruction on M4 (the
# 16x16 simdgroup MMA is M5+/GPUFamily10 at runtime; the 8x8 intrinsic
# SIGSEGVs the 1.0.0b1 compiler — fixed on modular main 2026-06-11).
# So v0 is a hand-vectorized SIMD kernel: lane j of a simdgroup owns
# K-row j of the 32-row tile for QK^T (SIMD8 fp16 loads, fp32 accum),
# online softmax keeps lane-replicated (m, l) per q row, and PV
# broadcasts p_j lane-by-lane while the 32 lanes split D columns.
#
...
comptime BQ = 16  # q rows per threadgroup
comptime BK = 32  # k/v rows per smem tile (== simdgroup width: lane j owns row j)
comptime TPB = 128  # threads per threadgroup (4 simdgroups)
comptime NW = TPB // 32  # simdgroups per threadgroup
comptime ROWS_PER_WARP = BQ // NW
comptime PAD = 8  # f16 elements of row padding (threadgroup-memory banking)
...
def fwd_kernel[
    ...
    comptime CH = D // 32  # O columns owned per lane
    comptime KROW = D + PAD  # padded smem row stride (f16 elements)
    ...
    # --- load this threadgroup's Q tile (BQ x D) once, SIMD8 vectors ---
    ...
    # m is kept in the log2 domain (matches the exp2 calls). o_acc is an
    # InlineArray of per-row SIMD chunks: every index is comptime, so
    # SROA keeps the rows in registers (the AIR backend has no
    # llvm.vector.insert, which rules out one big sliced SIMD).
    ...
    for tile in range(num_tiles):
        ...
        comptime for r in range(ROWS_PER_WARP):
            ...
            # o[qrow, lane-chunk] += p_j * v[j, lane-chunk] for all 32 j
...
def main() raises:
    ...
    if seq % BQ != 0 or seq % BK != 0:
        raise Error("--seq must be a multiple of 32 (v0 envelope)")
