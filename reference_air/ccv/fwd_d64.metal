...
using namespace metal;
  ...
  device half* L [[buffer(4)]],
    ...
    Q = Q + gid.z * Q_batch_stride + gid.y * 64;


    K = K + gid.z * K_batch_stride + gid.y / H_Hk_ratio * 64;


    V = V + gid.z * V_batch_stride + gid.y / H_Hk_ratio * 64;


    O = O + gid.z * O_batch_stride + gid.y * 64;
    ...
    L = L + (gid.z * Hq + gid.y) * R;
      ...
      simdgroup_matrix_storage<half> Q_sram[8];
  ...
  for (
    ...
    if (false && (
      ...
      auto Q_src = simdgroup_matrix_storage<half>
      ::apply_offset(
      ...
      if (sidx == 0) {
        ...
        event.async_copy<32, 32>(
          dst, tile_dst,
          src, Q_leading_dimension, tile_src,
          false);
    ...
    simdgroup_matrix_storage<float> O_sram[8];
