...
if [[ "$WINDOW" != 0 ]]; then CSUF="${CSUF}_window"; MSUF="${MSUF}_win"; CFLAG+=(--window "$WINDOW"); fi
if [[ "$SOFTCAP" != 0 ]]; then CSUF="${CSUF}_softcap"; MSUF="${MSUF}_scap"; CFLAG+=(--softcap "$SOFTCAP"); fi
